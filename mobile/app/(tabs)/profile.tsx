...
const THEME_OPTIONS: { k: ThemeMode; label: string }[] = [
  { k: 'system', label: uiText('跟随系统')! },
  { k: 'light', label: uiText('浅色')! },
  { k: 'dark', label: uiText('深色')! },
];
...
function Appearance({ t }: { t: Theme }) {
  ...
  return (
    <Card style={{ padding: 16 }}>
      <Text style={{ fontSize: 12, fontWeight: '700', color: t.tx3, letterSpacing: 0.5, marginBottom: 12 }}>{uiText('外观')}</Text>

      <Text style={{ fontSize: 13.5, fontWeight: '600', color: t.tx, marginBottom: 9 }}>{uiText('主题')}</Text>
      ...
      <Text style={{ fontSize: 13.5, fontWeight: '600', color: t.tx, marginTop: 16, marginBottom: 11 }}>{uiText('点缀色')}</Text>
...
const ABOUT_LINKS: { icon: string; label: string; sub: string; url: string }[] = [
  { icon: 'globe', label: uiText('官方网站')!, sub: 'monkeycode-ai.com', url: 'https://monkeycode-ai.com' },
  { icon: 'file', label: uiText('帮助文档')!, sub: 'monkeycode.docs.baizhi.cloud', url: 'https://monkeycode.docs.baizhi.cloud/' },
  { icon: 'github', label: uiText('GitHub 开源仓库')!, sub: 'chaitin/MonkeyCode', url: 'https://github.com/chaitin/MonkeyCode' },
];
...
function About({ t }: { t: Theme }) {
  ...
  const applyOtaNow = useCallback(() => {
    ...
    void downloadAndApplyOta().catch(() => {
      setOtaBusy(null);
      androidAlert('更新失败', '下载失败，请检查网络后重试。');
    });
  }, []);
  ...
  const onCheck = useCallback(async () => {
    ...
    if (app) {
      setOtaBusy(null);
      androidAlert('发现新版本', `新版本 v${app.version} 可用，需前往下载安装新版本。`, [
    ...
    setOtaBusy(null);
    if (r.status === 'disabled') { androidAlert('检查更新', '开发模式下不可用，正式包才会检查更新。'); return; }
    // error（OTA 服务未上线/网络异常，拿不到有效数据）视为已是最新，不向用户报错
    if (r.status === 'error' || r.status === 'none') { androidAlert('已是最新', `当前已是最新版本 ${verLine}。`); return; }
    androidAlert('发现新版本', '有新版本可用，是否立即更新？\n（更新后将自动重启）', [
  ...
  return (
    <Card style={{ padding: 16 }}>
      <Text style={{ fontSize: 12, fontWeight: '700', color: t.tx3, letterSpacing: 0.5, marginBottom: 13 }}>{uiText('关于')}</Text>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
        ...
        <View style={{ flex: 1, minWidth: 0 }}>
          ...
          <Text style={{ fontSize: 12.5, color: t.tx3, marginTop: 2 }}>{uiText('开源 AI 编程助手 · 长亭科技')}</Text>
        </View>
      </View>
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose} statusBarTranslucent>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1, justifyContent: 'flex-end' }}>
        ...
        <View style={{ backgroundColor: t.bg2, borderTopLeftRadius: 24, borderTopRightRadius: 24, borderTopWidth: StyleSheet.hairlineWidth, borderColor: t.line2, paddingHorizontal: spacing.pad, paddingBottom: insets.bottom + 16 }}>
          ...
          <Text style={{ fontSize: 18, fontWeight: '800', color: t.tx }}>{uiText('绑定邮箱')}</Text>
          <Text style={{ fontSize: 13, color: t.tx2, fontWeight: '600', marginTop: 18, marginBottom: 8 }}>{uiText('邮箱地址')}</Text>
          <TextInput
            value={email}
          ...
          <View style={{ flexDirection: 'row', gap: 10, marginTop: 18 }}>
            ...
            >
              ...
              <Text style={{ color: t.acInk, fontSize: 14.5, fontWeight: '800' }}>{uiText('发送验证邮件')}</Text>
            </Pressable>
          </View>
        </View>
...
function CheckinButton({ checkedIn, busy, onPress, t }: { checkedIn: boolean | null; busy: boolean; onPress: () => void; t: Theme }) {
  ...
  if (checkedIn) {
    return (
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, height: 38, paddingHorizontal: 15, borderRadius: 99, backgroundColor: t.bg3 }}>
        ...
        <Text style={{ color: t.tx2, fontSize: 13.5, fontWeight: '700' }}>{uiText('今日已签到')}</Text>
      </View>
    );
  }
  // 可签到：主按钮
  return (
    <Pressable onPress={onPress} disabled={busy} style={({ pressed }) => [{ flexDirection: 'row', alignItems: 'center', gap: 6, height: 38, paddingHorizontal: 16, borderRadius: 99, backgroundColor: t.ac }, pressed && { transform: [{ scale: 0.96 }] }, busy && { opacity: 0.7 }]}>
      ...
      <Text style={{ color: t.acInk, fontSize: 13.5, fontWeight: '700' }}>{busy ? uiText('签到中') : uiText(`签到 +${CHECKIN_REWARD}`)}</Text>
    </Pressable>
  );
}
...
export default function ProfileScreen() {
  ...
  const handleBindEmail = useCallback(async () => {
    ...
    if (!EMAIL_RE.test(nextEmail)) {
      androidAlert('邮箱格式不正确', '请输入有效的邮箱地址。');
      return;
    }
    ...
    try {
      ...
      setBindEmailOpen(false);
      setBindEmail('');
      androidAlert('验证邮件已发送', `请前往 ${nextEmail} 查收验证邮件，完成验证后邮箱会显示在本页。`);
    } catch (e) {
      androidAlert('绑定邮箱失败', e instanceof Error && e.message ? e.message : '请稍后重试');
    } finally {
  ...
  const onDeleteAccount = () => {
    androidAlert('注销账号', '注销后，你的账号及全部数据将被永久删除，无法恢复；Apple 登录的授权也会一并撤销。', [
      { text: '取消', style: 'cancel' },
      {
        text: '继续',
        style: 'destructive',
        onPress: () => androidAlert('确认永久删除？', '此操作不可撤销。', [
          { text: '取消', style: 'cancel' },
          {
            text: '永久删除',
            style: 'destructive',
            onPress: async () => {
              ...
              try {
                ...
                setBusy(false);
                androidAlert('注销失败', e instanceof Error && e.message ? e.message : '请稍后重试。如多次失败，请通过官网联系我们处理。');
              }
            },
          },
        ]),
        ...
        <View style={{ paddingHorizontal: spacing.pad, paddingTop: 12, gap: spacing.gap }}>
            ...
            <View style={{ flex: 1, minWidth: 0 }}>
              ...
              <View style={{ marginTop: 7, gap: 5 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 7, minWidth: 0 }}>
                  ...
                  <Text numberOfLines={1} style={{ flex: 1, minWidth: 0, fontSize: 12.5, color: t.tx3, fontWeight: '500' }}>{uiText(email || '未绑定邮箱')}</Text>
                  {!email ? (
                    <Pressable onPress={() => setBindEmailOpen(true)} hitSlop={8} style={({ pressed }) => [{ paddingHorizontal: 4, paddingVertical: 2 }, pressed && { opacity: 0.55 }]}>
                      <Text style={{ color: t.acTx, fontSize: 12.5, fontWeight: '700' }}>{uiText('绑定')}</Text>
                    </Pressable>
                  ) : null}
                </View>
                {user?.id ? (
          ...
          <Card style={{ padding: 16 }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
              <View style={{ flex: 1, minWidth: 0 }}>
                <Text style={{ fontSize: 12.5, color: t.tx3, fontWeight: '500' }}>{uiText('积分余额')}</Text>
                {loadingWallet
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 9, marginTop: 14, paddingTop: 13, borderTopWidth: StyleSheet.hairlineWidth, borderColor: t.line }}>
              ...
              <View style={{ flex: 1, minWidth: 0 }}>
                <Text style={{ fontSize: 13, color: t.tx2, fontWeight: '600' }}>{uiText(`已邀请 ${inviteCount} 人`)}</Text>
                <Text style={{ fontSize: 11.5, color: t.tx3, marginTop: 1 }}>{uiText(`每邀请一位 +${INVITE_REWARD.toLocaleString('zh-CN')} 积分`)}</Text>
              </View>
              {inviteLink ? (
                <Pressable onPress={() => copy(inviteLink, '邀请链接已复制，分享给好友')} style={({ pressed }) => [{ flexDirection: 'row', alignItems: 'center', gap: 5, height: 32, paddingHorizontal: 13, borderRadius: 99, backgroundColor: t.acGhost }, pressed && { opacity: 0.6 }]}>
                  ...
                  <Text style={{ color: t.acTx, fontSize: 12.5, fontWeight: '700' }}>{uiText('邀请好友')}</Text>
                </Pressable>
              ) : null}
            </View>
          </Card>
          ...
          <Card style={{ paddingTop: 14, paddingBottom: 2 }}>
            <Text style={{ fontSize: 12, fontWeight: '700', color: t.tx3, letterSpacing: 0.5, paddingHorizontal: 16, marginBottom: 2 }}>{uiText('集成')}</Text>
          ...
          <Pressable onPress={onLogout} disabled={busy} style={({ pressed }) => [{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 9, paddingVertical: 15, borderRadius: 18, backgroundColor: t.bg2 }, t.shCard, pressed && { opacity: 0.7 }]}>
            ...
            <Text style={{ color: t.red, fontSize: 15.5, fontWeight: '600' }}>{uiText('退出登录')}</Text>
          </Pressable>
          ...
          {appleSession ? (
            <Pressable onPress={onDeleteAccount} disabled={busy} style={({ pressed }) => [{ alignItems: 'center', paddingVertical: 10 }, pressed && { opacity: 0.6 }]}>
              <Text style={{ color: t.tx3, fontSize: 13 }}>{uiText('注销账号')}</Text>
            </Pressable>
          ) : null}
